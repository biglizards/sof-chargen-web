use crate::data::careers::{CareerTableEntry, CareerTableStar, get_affiliation, get_careers, Career};
...
pub(crate) gen fn roll_affiliation(backend: Backend, mut disadvantage: usize) -> Choice {
...
pub(crate) gen fn change_rank(backend: Backend, rank: i8) -> Choice {
    let char = backend.character();
    let loc = char.birth_location.clone().unwrap();
